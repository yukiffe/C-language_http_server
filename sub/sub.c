...
int main() {
	...
	_server_address.sin_addr.s_addr = htonl(INADDR_ANY); // 프로토콜, TCP로 비연결성 유지
	...
	while (true) { // 통신을 지속적으로 반복
        ...
        // 클라이언트와 연결 후 끊는 경우 http 통신 취급
        // http는 전체를 주고받아 연결 유지가 필요 x
        // 연결 해제 없이 그대로 동작 ( 연결유지는 socket 통신 )
	}
}
